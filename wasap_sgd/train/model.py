...
class SETMPIModel(object):
    ...
    def _back_prop(self, z, a, masks, y_true):
        ...
        # delta output layer
        #这里delta是误差y_pre-y_true
        delta = self.loss.delta(y_true, a[self.n_layers])
        #直接吧最后一层的权重转换为coo_matrix稀疏矩阵
        ...
        for i in reversed(range(2, self.n_layers)):
            ...
            if keep_prob != 1:
                ...
                if len(masks)!=0: #whether dropout
                    delta = delta * masks[i]
                delta /= keep_prob
            else:
                delta = (delta @ self.w[i].transpose()) * self.activations[i].prime(z[i])
    ...
    def train_on_batch(self, x, y):
        #z, a, masks 是feed_forward的返回
        z, a, masks = self._feed_forward(x, True)# drop True
        return self._back_prop(z, a, masks, y)
    #compute loss return loss and accuracy
    ...
    def to_label(self, y_pre):

        for j in range(y_pre.shape[0]):
            index = np.argmax(y_pre[j])
            y_pre[j][index] = 1
